    ...
    setupFilterListeners() {
        ...
        document.querySelectorAll('.dietary-toggle').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                btn.classList.toggle('active');  // Toggle button state FIRST
                this.toggleDietaryFilter(dietary);  // Then apply the filter
            });
        });
    }
    ...
    toggleDietaryFilter(dietary) {
        ...
        recipeLinks.forEach(link => {
            ...
            if (recipe && recipe.tags) {
                ...
                // FIXED LOGIC: If filter is active, show only matching recipes. If not active, show all.
                const isFilterActive = activeFilters.includes(dietary);
                
                if (isFilterActive) {
                    // Filter is ACTIVE: show only recipes that match this filter
                    if (hasDietaryTag) {
                        parentLi.style.display = 'list-item';  // Show with bullet point
                        console.log(`Showing ${recipeId} (matches active filter ${dietary})`);
                        if (sectionId) {
                            sectionVisibility[sectionId] = true;
                        }
                    } else {
                        parentLi.style.display = 'none';  // Hide completely
                        console.log(`Hiding ${recipeId} (doesn't match active filter ${dietary})`);
                        if (sectionId && !sectionVisibility[sectionId]) {
                            sectionVisibility[sectionId] = false;
                        }
                    }
                } else {
                    // Filter is INACTIVE: show all recipes
                    parentLi.style.display = 'list-item';  // Show with bullet point
                    console.log(`Showing ${recipeId} (filter ${dietary} is inactive, showing all)`);
                    if (sectionId) {
                        sectionVisibility[sectionId] = true;
                    }
                }

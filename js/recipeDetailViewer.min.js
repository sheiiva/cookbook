class RecipeDetailViewer {
    ...
    displayRecipe() {
        ...
        // Update page header with recipe name
    ...
    renderInstructions(recipe) {
        // Handle instruction_blocks structure (like moussaka)
        if (recipe.instruction_blocks) {
            let html = '';
            for (const [key, block] of Object.entries(recipe.instruction_blocks)) {
                html += `
    ...
    renderIngredients(recipe) {
        ...
        // Handle main ingredients array
        if (recipe.ingredients && recipe.ingredients.length > 0) {
            html += '<ul>';
            html += recipe.ingredients.map(ingredient => `<li>${ingredient}</li>`).join('');
            html += '</ul>';
        }
        
        // Handle ingredient sections (like béchamel for moussaka)

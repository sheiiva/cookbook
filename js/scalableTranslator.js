...
class ScalableTranslator {
    ...
    updateRecipeContent() {
        ...
        // Update recipe content if we're on a recipe page
        if (window.dynamicRecipeRenderer && this.translatedContent.categories) {
            console.log('Updating recipe content with translated data...');
            // Find the current recipe and update it with translated content
            const urlParams = new URLSearchParams(window.location.search);
            const recipeId = urlParams.get('id');
            
            if (recipeId) {
                // Find the translated recipe data
                let translatedRecipe = null;
                for (const category of Object.values(this.translatedContent.categories)) {
                    const foundRecipe = category.recipes.find(r => r.id === recipeId);
                    if (foundRecipe) {
                        translatedRecipe = foundRecipe;
                        break;
                    }
                }
                
                if (translatedRecipe) {
                    console.log('Found translated recipe data, updating...');
                    window.dynamicRecipeRenderer.updateWithTranslatedContent(translatedRecipe);
                }
            }
        }
    }
